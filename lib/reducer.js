    ...
    case types.EDIT_MANY_CONTENTS:
      {
        ...
        return state.updateIn(_contentsPath, action.contents);
      }
    ...
    case types.REORDER_MANY_CONTENTS:
      {
        var _ret = function () {
          ...
          return {
            v: state.setIn(contentsPath, updatedContents)
          };
        }();
    ...
    case types.VALIDATE_MANY:
      {
        if (action.validate) {
          var _ret2 = function () {
            var errorsPath = action.path.concat([_schemaMapping2.default.many.errors]);
            var contentsPath = action.path.concat([_schemaMapping2.default.many.contents]);
            var contents = state.getIn(contentsPath);

            return {
              v: state.updateIn(errorsPath, function (val) {
                // Run contents through the validator function
                return _immutable2.default.fromJS(action.validate(contents.toJS()));
              })
            };
          }();
