...
function reducer(state, action) {
  switch (action.type) {
    ...
    case types.EDIT_MANY_CHILD_FORMS_CHILDREN:
      {
        var childrenPath = action.path.concat([_schemaMapping2.default.manyChildForms.children]);
        return state.updateIn(childrenPath, action.children);
      }

    case types.REORDER_MANY_CHILD_FORMS_CHILDREN:
      {
        var path = action.path.concat([_schemaMapping2.default.manyChildForms.children]);
        var children = state.getIn(path);
        var updatedChildren = _immutable2.default.fromJS(action.order).map(function (index) {
    ...
    case types.VALIDATE_MANY_CHILD_FORMS:
      {
        if (action.validate) {
          ...
          var _childrenPath = action.path.concat([_schemaMapping2.default.manyChildForms.children]);
          var _children = state.getIn(_childrenPath);

          return state.updateIn(_errorsPath2, function (val) {
            // Run contents through the validator function
            return _immutable2.default.fromJS(action.validate(_children.toJS()));
          });
        }
        return state;
      }
